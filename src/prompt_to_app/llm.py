...
@dataclass
class OllamaLLM:
    model:str="qwen2.5-coder:7b"; base_url:str="http://127.0.0.1:11434"
    def generate(self,system,prompt):
        ...
        with request.urlopen(req,timeout=120) as resp: return str(json.loads(resp.read().decode())["response"])

@dataclass
class OpenAICompatibleLLM:
    api_key:str; model:str="gpt-4.1-mini"; base_url:str="https://api.openai.com/v1"
    def generate(self,system,prompt):
        ...
        try:
            with request.urlopen(req,timeout=120) as resp: data=json.loads(resp.read().decode())
            return str(data["choices"][0]["message"]["content"])
        except Exception as exc: raise CloudLLMError(str(exc)) from exc

def cloud_llm_from_env():
    key=os.getenv("LLM_API_KEY") or os.getenv("OPENAI_API_KEY")
    if not key: raise CloudLLMError("LLM_API_KEY or OPENAI_API_KEY is required")
