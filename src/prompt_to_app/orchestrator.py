...
def build(prompt: str, output_dir: str|Path="./generated-app", model: str="qwen2.5-coder:7b",
    ...
    research_data = research.run(prompt).as_dict() if research is not None else None
    app=plan(prompt,model=model,base_url=base_url,llm=llm,research=research_data)
    project_dir=generate(app,output_dir,model=model,base_url=base_url,llm=llm,research=research_data)
