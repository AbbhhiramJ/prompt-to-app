...
applications never need provider credentials.
"""
...
class Deployer(Protocol):
    def deploy_from_github(
        self, owner: str, repo_id: str, branch: str, name: str
    ) -> str: ...
...
class PromptToAppWorkflow:
    """Core local build pipeline.

    Repository and deployment operations intentionally stay outside this class.
    The host agent can inject them without exposing credentials to the app.
    """
    ...
    visual_audit: Callable[
        [Mapping[str, str]], Sequence[str]
    ] | None = None
    repair: Callable[
        [str, Mapping[str, object], Mapping[str, str], Sequence[str]],
        Mapping[str, str],
    ] | None = None

    def prepare(
        self, prompt: str
    ) -> tuple[BuildState, Mapping[str, str]]:
        if not prompt.strip():
