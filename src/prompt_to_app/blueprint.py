...
def _from_data(data:Mapping[str,object], sources:list[Mapping[str,object]]):
    fields=AppBlueprint.__dataclass_fields__
    clean={k:data[k] for k in fields if k in data and k!="evidence"}
...
def synthesize(prompt:str,research:Mapping[str,object],synthesizer:Synthesizer|None=None,llm:LLM|None=None,model:str="qwen2.5-coder:7b")->AppBlueprint:
    sources=list(research.get("sources",[]))
    if synthesizer is not None:
        try: return _from_data(dict(synthesizer(prompt,research)),sources)
    ...
    bp=AppBlueprint(
      ...
      onboarding=["Keep setup short","Offer sensible starter items","Ask only for information needed to begin"],
