...
def _fallback_files(plan):
    if plan.name=="habit-tracker":
        return {"index.html":"<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Habit Tracker</title><link rel='stylesheet' href='style.css'></head><body><main><h1>Habit Tracker</h1><p>Build consistency one habit at a time.</p><button id='add-habit'>Add habit</button><ul id='habits'></ul><p id='status'></p></main><script src='app.js'></script></body></html>","style.css":"body{font-family:system-ui,sans-serif;max-width:760px;margin:80px auto;padding:24px}button{padding:10px 16px}","app.js":"const b=document.querySelector('#add-habit'),l=document.querySelector('#habits'),s=document.querySelector('#status');b.onclick=()=>{const i=document.createElement('li');i.textContent='New habit';l.append(i);s.textContent='Habit added';}"}
