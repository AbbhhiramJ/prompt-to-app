...
from .llm import CloudLLMError, cloud_llm_from_env
...
def main()->int:
    ...
    parser.add_argument("--cloud",action="store_true",help="Use configured cloud LLM and deploy to Vercel")
    parser.add_argument("--cloud-name",default="prompt-to-app")
    parser.add_argument("--port",type=int,default=8000)
    parser.add_argument("--max-repairs",type=int,default=2)
    args=parser.parse_args()
    try:
        llm=cloud_llm_from_env() if args.cloud else None
        plan,project_dir,errors,url,repairs=build(
            args.prompt,args.output,model=args.model,base_url=args.ollama_url,
            serve=args.serve and not args.cloud,browser=args.browser and not args.cloud,
            port=args.port,max_repairs=args.max_repairs,llm=llm)
